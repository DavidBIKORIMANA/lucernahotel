...
.sr-hero-bg { position:absolute; inset:0; background:linear-gradient(139deg,#0c62c3 14.24%,#034ea2 75.61%); }
...
.sr-hero-title em { font-style:italic; font-weight:400; }
.sr-dates-strip {
...
.sr-dates-strip svg { width:16px; height:16px; stroke:var(--brand-pale); }
...
.sr-avail-badge {
    ...
    background:rgba(34,197,94,.9); color:var(--white);
}
...
.sr-card-btn {
    ...
    padding:9px 18px; background:var(--brand); color:var(--white);
    border:none; border-radius:2px; text-decoration:none;
    transition:all .25s;
}
.sr-card-btn:hover { background:var(--brand-light); }
.sr-empty {
...
.sr-back-link {
    ...
    font-family:var(--f-body); font-size:13px; color:var(--brand);
...
<section class="sr-hero">
    ...
    <div class="sr-hero-content">
        ...
        <h1 class="sr-hero-title">Available <em>Rooms</em></h1>
        <?php if(old('check_in') && old('check_out')): ?>
        <div class="sr-dates-strip">
            ...
            <span><strong><?php echo e(old('check_in')); ?></strong> — <strong><?php echo e(old('check_out')); ?></strong></span>
        </div>
        <?php endif; ?>
    </div>
</section>
    ...
    <div class="sr-grid">
        ...
        <div class="sr-card">
            <div class="sr-card-img">
                <a href="<?php echo e(route('search_room_details', $item->id.'?check_in='.old('check_in').'&check_out='.old('check_out').'&persion='.old('persion'))); ?>">
                    <img src="<?php echo e(asset('upload/roomimg/'.$item->image)); ?>" alt="<?php echo e($item->type->name ?? 'Room'); ?>" loading="lazy">
                </a>
                <span class="sr-avail-badge"><?php echo e($av_room); ?> Available</span>
            </div>
            <div class="sr-card-body">
                <div class="sr-card-name"><?php echo e($item->type->name ?? 'Room'); ?></div>
                ...
                <div class="sr-card-footer">
                    <div class="sr-card-price">$ <?php echo e(number_format($item->price, 0)); ?> <span>/ night</span></div>
                    <a href="<?php echo e(route('search_room_details', $item->id.'?check_in='.old('check_in').'&check_out='.old('check_out').'&persion='.old('persion'))); ?>" class="sr-card-btn">Reserve</a>
                </div>
            </div>
        </div>
        <?php else: ?>
            <?php $empty_array[] = $item->id; ?>
        <?php endif; ?>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>

        <?php if(count($rooms) == count($empty_array)): ?>
        <div class="sr-empty">
            ...
            <a href="<?php echo e(url('/')); ?>" class="sr-back-link">← Try Different Dates</a>
        </div>
        <?php endif; ?>
    </div>
</div>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('frontend.main_master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\workspace\htdocs\lucerna\resources\views/frontend/room/search_room.blade.php ENDPATH**/ ?>